...
import { 
  Plus, Search, Tags, Trash2, Edit, ChevronLeft, ChevronRight, Image as ImageIcon, LayoutGrid, Store, Globe
} from 'lucide-react';
...
const ITEMS_PER_PAGE = 12; // 🚀 Aumentamos por página al ser cards más pequeñas
...
export default function CategoriesPage() {
  ...
  // 🚀 Función auxiliar para obtener rama visual
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter(c => {
      ...
      if (codeFilter === 'GENERAL') matchesCode = isGlobal;
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation(); // Evitar que se abra el modal al clickear eliminar
      ...
      <div className="flex-1 flex flex-col min-h-[400px]">
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {[1, 2, 3, 4, 5, 6].map(i => <Skeleton key={i} className="aspect-square w-full rounded-2xl bg-white border border-slate-200 shadow-sm" />)}
          </div>
        ) : paginatedCategories.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center bg-white rounded-2xl border border-dashed border-slate-200 shadow-sm py-20">
            <Tags className="w-12 h-12 text-slate-200 mb-3" />
            <p className="font-medium text-sm text-slate-500">No se encontraron categorías en esta sección.</p>
            <Button variant="link" onClick={() => { setSearchTerm(''); setCodeFilter('ALL'); }} className="text-blue-500">Limpiar filtros</Button>
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-4">
            {paginatedCategories.map((cat: Category) => {
              ...
              return (
                ...
                >
                  {/* 🚀 Botón de Eliminar Oculto (Aparece en Hover) */}
                  {canEditThis && (
                    <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="destructive" size="icon" className="h-7 w-7 rounded-full shadow-md bg-red-500 hover:bg-red-600" onClick={(e) => handleDelete(cat.id, e)}>
                  ...
                  <div className="p-3 sm:p-4 flex flex-col gap-1.5 flex-1 bg-white">
                    ...
                    <div className="mt-auto pt-2 flex flex-col xl:flex-row xl:items-center justify-between gap-2">
                      ...
                      <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-md border border-slate-100 w-max whitespace-nowrap">
        ...
        {/* 🚀 PAGINACIÓN PLANA */}
        {totalPages > 1 && (
          <div className="px-1 py-4 mt-2 flex justify-between items-center">
            <span className="text-[11px] font-medium text-slate-500">Página {currentPage} de {totalPages}</span>
            <div className="flex gap-1.5">
              <Button variant="outline" className="h-8 px-2.5 bg-white border-slate-200 text-slate-600 hover:bg-slate-100 rounded-lg shadow-sm" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}><ChevronLeft className="w-4 h-4" /></Button>
              <Button variant="outline" className="h-8 px-2.5 bg-white border-slate-200 text-slate-600 hover:bg-slate-100 rounded-lg shadow-sm" onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}><ChevronRight className="w-4 h-4" /></Button>
            </div>
          </div>
        )}
      </div>
