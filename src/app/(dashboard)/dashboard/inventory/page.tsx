...
function InventoryPageContent() {
  ...
  // Column definitions for Transfers ResponsiveTable
  const transfersColumns: ColumnDef<StockTransfer>[] = [
    {
      key: 'status',
      header: 'Estado',
      mobileProminent: true,
      ...
      <div className="flex flex-col flex-1 min-h-[400px] border-none overflow-hidden relative">
        ...
        {activeTab === 'kardex' ? (
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <button
                      ...
                      className={`w-full h-10 px-3 text-xs font-bold rounded-lg border transition-all flex items-center justify-between ${
                        typeFilter !== 'ALL'
                    ...
                    {showTypeFilter && (
                      <>
                        ...
                        <div className="absolute top-12 left-0 right-0 bg-white border border-slate-200 shadow-xl rounded-xl p-1.5 z-50 flex flex-col gap-0.5 animate-in fade-in zoom-in-95 duration-100">
                  ...
                  <div className="relative flex-1">
                    ...
                    {showBranchFilter && (
                      <>
                        ...
                        <div className="absolute top-12 left-0 right-0 bg-white border border-slate-200 shadow-xl rounded-xl p-1.5 z-50 flex flex-col gap-0.5 animate-in fade-in zoom-in-95 duration-100 max-h-60 overflow-y-auto">
          ...
          <>
            ...
            {/* RESPONSIVE TABLE/CARDS FOR TRANSFERS */}
            {isMobile ? (
              <div className="px-4 py-3 space-y-2">
                {!transfers ? (
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {paginatedTransfers.map((transfer) => {
                      const isExpanded = expandedTransferCards.has(transfer.id);
                      const toggleExpand = () => {
                        setExpandedTransferCards((prev) => {
                          const newSet = new Set(prev);
                          if (newSet.has(transfer.id)) {
                      ...
                      return (
                        <div key={transfer.id} className="bg-white border border-slate-200 rounded-lg p-4 space-y-3 shadow-sm">
                        {/* Status Badge */}
                        <div className="flex items-center justify-between">
                          <Badge className={`text-[9px] font-black px-2 py-0.5 h-5 shadow-none border ${statusConfig[transfer.status].color}`}>
                            {statusConfig[transfer.status].label}
                          ...
                          <span className="text-[10px] text-slate-500">
                            {new Date(transfer.createdAt).toLocaleDateString('es-PE', { day: '2-digit', month: 'short', year: 'numeric' })}
                          </span>
                        </div>
                        ...
                        <div>
                          ...
                          {isExpanded && (
                            <div className="mt-2 space-y-1 pl-6 border-l-2 border-slate-200">
                              {transfer.items.map((item) => (
                        ...
                        <button
                          ...
                          className="w-full flex items-center justify-center gap-1 text-sm text-primary hover:text-primary/80 transition-colors min-h-[44px] border-t border-slate-100 pt-2"
                        >
