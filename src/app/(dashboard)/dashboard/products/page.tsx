...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter(p => {
      ...
      if (codeFilter === 'GENERAL') {
        ...
        matchesCode = p.category?.ecommerceCode === codeFilter;
      }
  ...
  const downloadBarcodePNG = async () => {
    ...
    try {
      ...
      toast.error('Error al generar la imagen. Revisa la consola.', { id: 'barcode-toast' });
    }
  };
                    ...
                    <tr key={product.id} className={`hover:bg-slate-50 transition-colors group ${!product.active ? 'opacity-75 bg-slate-50' : ''}`}>
                          ...
                          <div>
                            ...
                            {(product.barcode || product.code) && (
                              <div className="flex items-center gap-1 text-[10px] text-slate-400 font-mono mt-0.5">
                                <BarcodeIcon className="w-3 h-3" /> {product.barcode || product.code}
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-3">
                        <div className="flex flex-col items-start gap-1">
                          <Badge variant="secondary" className="bg-slate-100 text-slate-600 font-normal truncate max-w-[150px]" title={product.category?.name}>
                      ...
                      <td className="px-6 py-3">
                        ...
                        {hasWholesale && (
                          <div className="mt-1">
                            <span className="text-[10px] text-emerald-700 font-bold bg-emerald-100/80 px-1.5 py-0.5 rounded border border-emerald-200">
                              Mayorista: S/ {Number(product.wholesalePrice).toFixed(2)}
                            </span>
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-3">
                        <div className="flex flex-col gap-1">
                          <Badge variant="outline" className={Number(product.stock) <= 0 ? 'border-red-200 text-red-600 bg-red-50 text-[10px]' : Number(product.stock) <= Number(product.minStock) ? 'border-amber-200 text-amber-600 bg-amber-50 text-[10px]' : 'border-blue-200 text-blue-700 bg-blue-50 text-[10px]'}>
                            Web: {product.stock}
                          </Badge>
                      ...
                      <td className="px-6 py-3 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                          ...
                          {(product.barcode || product.code) && (
                            <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-600 hover:bg-slate-200" onClick={() => setBarcodeProduct(product)} title="Ver Etiqueta">
