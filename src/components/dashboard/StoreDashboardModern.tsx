    ...
    <div className="flex flex-col h-full w-full gap-5 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 min-h-0 overflow-hidden">
        ...
        <div className="flex flex-col gap-3 min-h-0 overflow-y-auto custom-scrollbar">
              ...
              {stats.todayVsYesterday && (
                <div className={`flex items-center gap-1 px-2 py-1 rounded-lg ${
                  ...
                  <span className={`text-[10px] font-black ${
                    stats.todayVsYesterday.revenueChange >= 0 ? 'text-emerald-700' : 'text-red-700'
                  }`}>
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm hover:shadow-md transition-all">
              ...
              <p className="text-lg font-black text-slate-900 tabular-nums">S/ {stats.totalCost.toFixed(2)}</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm hover:shadow-md transition-all">
              ...
              <p className="text-lg font-black text-emerald-600 tabular-nums">S/ {stats.totalProfit.toFixed(2)}</p>
              {stats.todayVsYesterday && (
                <div className="flex items-center gap-1 mt-1.5">
                  {stats.todayVsYesterday.profitChange >= 0 ? (
                  ...
                  <span className={`text-[10px] font-bold ${
                    stats.todayVsYesterday.profitChange >= 0 ? 'text-emerald-600' : 'text-red-600'
                  }`}>
                    {Math.abs(stats.todayVsYesterday.profitChange).toFixed(1)}%
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 shrink-0">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm hover:shadow-md transition-all">
              ...
              <p className="text-xl font-black text-slate-900 tabular-nums">{stats.profitMargin.toFixed(1)}%</p>
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm hover:shadow-md transition-all">
              ...
              <p className="text-xl font-black text-slate-900 tabular-nums">{stats.totalOrders}</p>
              {stats.todayVsYesterday && (
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className={`text-[9px] font-bold ${
                    stats.todayVsYesterday.ordersChange >= 0 ? 'text-emerald-600' : 'text-red-600'
                  }`}>
          ...
          <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border-2 border-slate-200 p-4 shadow-sm shrink-0">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-black text-blue-600">S/ {stats.totalRevenue.toFixed(2)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-600">Costos</span>
                <span className="text-sm font-black text-red-600">- S/ {stats.totalCost.toFixed(2)}</span>
              </div>
              <div className="h-px bg-slate-300" />
              <div className="flex items-center justify-between">
                <span className="text-sm font-black text-slate-900">Ganancia Neta</span>
                <span className="text-base font-black text-emerald-600">S/ {stats.totalProfit.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border-2 border-slate-200 p-4 shadow-sm hover:shadow-md transition-all flex flex-col min-h-0 overflow-hidden">
          <div className="flex items-center justify-between mb-3 shrink-0">
            ...
            <div className="px-2 py-0.5 bg-slate-100 rounded-lg">
              <span className="text-[9px] font-black text-slate-700">{stats.topProducts?.length || 0}</span>
            </div>
          </div>
                ...
                return (
                  <div key={product.id} className="group relative">
                    ...
                    <div 
                      ...
                      style={{ width: `${(product.profit / stats.totalProfit) * 100}%` }}
                    />
        ...
        <div className="flex flex-col gap-3 min-h-0 overflow-y-auto custom-scrollbar">
          ...
          {isOwner && stats.salesByBranch && stats.salesByBranch.length > 1 ? (
              ...
              <div className="bg-white rounded-xl border-2 border-slate-200 p-3 shadow-sm hover:shadow-md transition-all shrink-0">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between p-2 bg-red-50 rounded-lg">
                    ...
                    <p className="text-sm font-black text-red-600">S/ {stats.totalCost.toFixed(2)}</p>
                  </div>
                  <div className="flex items-center justify-between p-2 bg-slate-50 rounded-lg">
                    ...
                    <p className="text-sm font-black text-slate-700">
                      S/ {stats.totalOrders > 0 ? (stats.totalCost / stats.totalOrders).toFixed(2) : '0.00'}
                    </p>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="p-2 bg-emerald-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-black text-emerald-600">
                        S/ {stats.totalOrders > 0 ? (stats.totalProfit / stats.totalOrders).toFixed(2) : '0.00'}
                      </p>
                    </div>
                  ...
                  <div className="p-2 bg-purple-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-black text-purple-600">
                        S/ {stats.totalSales > 0 ? (stats.totalProfit / stats.totalSales).toFixed(2) : '0.00'}
                      </p>
                    </div>
                  ...
                  <div className="p-2 bg-amber-50 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-sm font-black text-amber-600">{stats.totalSales}</p>
                    </div>
              ...
              <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl border-2 border-slate-200 p-3 shadow-sm shrink-0">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-black text-slate-900">{stats.totalOrders}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-semibold text-slate-600">Ticket Promedio</span>
                    <span className="text-xs font-black text-slate-900">S/ {stats.averageTicket.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-semibold text-slate-600">Items/Venta</span>
                    <span className="text-xs font-black text-slate-900">
                      {stats.totalOrders > 0 ? (stats.totalSales / stats.totalOrders).toFixed(1) : '0'}
                    </span>
                  </div>
                  <div className="h-px bg-slate-300" />
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-slate-900">Eficiencia</span>
                    <span className="text-sm font-black text-emerald-600">{stats.profitMargin.toFixed(1)}%</span>
                  </div>
                </div>
              </div>
            </>
