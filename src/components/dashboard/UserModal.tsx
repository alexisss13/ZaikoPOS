...
import { 
  ...
  Wallet, PieChart, Users, EyeOff, ArrowRightLeft, FileWarning, DollarSign, Globe, ChevronDown, Info
} from 'lucide-react';
import { useAuth } from '@/context/auth-context';
import { ScrollArea } from '@/components/ui/scroll-area';
// 🚀 NUEVO: Importamos el sistema de Tooltips nativo de la UI
...
// Creamos un tipo estricto para el acordeón
...
export function UserModal({ isOpen, onClose, onSuccess, userToEdit }: UserModalProps) {
  ...
  // 🚀 FIX TYPESCRIPT: Definido con el tipo que creamos
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', password: '', role: 'CASHIER', businessId: 'NONE', branchId: 'NONE',
  });
  ...
  // 🚀 COMPONENTE CON TOOLTIPS REALES DE UI Y ESTRUCTURA LIMPIA
  const PermissionRow = ({ label, desc, stateKey, type = 'default' }: { label: string, desc: string, stateKey: keyof typeof permissions, type?: 'default'|'warning'|'critical' }) => {
    ...
    return (
      <div className={`flex items-center justify-between p-3 sm:px-4 sm:py-3 border-b border-slate-100 last:border-0 transition-colors ${bgColors[type]}`}>
        <div className="flex items-center gap-2">
          <Label className={`text-xs sm:text-sm font-semibold cursor-pointer ${textColors[type]}`} onClick={() => handlePermissionToggle(stateKey)}>
            {label}
          </Label>
          <TooltipProvider delayDuration={100}>
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="cursor-help text-slate-300 hover:text-blue-500 transition-colors p-1 -ml-1">
                  <Info className="w-4 h-4" />
                </div>
              </TooltipTrigger>
              <TooltipContent side="top" className="bg-slate-900 text-slate-50 text-xs px-3 py-2 max-w-[200px] text-center font-medium border-slate-800">
                <p>{desc}</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
  ...
  // Helper para manejar el acordeón
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="w-[95vw] sm:max-w-5xl p-0 overflow-hidden bg-slate-50 font-sans">
        
        <DialogHeader className="px-4 py-3 sm:px-6 sm:py-4 bg-white border-b border-slate-200 shadow-sm flex flex-row items-center justify-between">
        ...
        <ScrollArea className="max-h-[85vh] sm:max-h-[75vh] px-4 py-4 sm:px-6 sm:py-6 overflow-x-hidden">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select value={formData.branchId} onValueChange={(v) => setFormData(p => ({...p, branchId: v}))} disabled={formData.role === 'SUPER_ADMIN'}>
                        <SelectTrigger className={`h-9 text-sm focus-visible:ring-blue-500 ${formData.branchId !== 'NONE' ? 'bg-blue-50/40 border-blue-200' : 'bg-slate-50 border-slate-200'}`}><SelectValue placeholder="Elige base" /></SelectTrigger>
                        <SelectContent>
                          ...
                          {branches?.map((branch) => (
                            <SelectItem key={branch.id} value={branch.id}>
                              <div className="flex items-center gap-2.5">
                                {branch.logoUrl ? <img src={branch.logoUrl} alt={branch.name} className="w-4 h-4 rounded-sm object-cover border border-slate-200" /> : <Store className="w-3.5 h-3.5 text-blue-500" />}
                ...
                {formData.role !== 'SUPER_ADMIN' ? (
                  <div className="space-y-3">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                      <button type="button" onClick={() => toggleSection('inventory')} className={`w-full px-4 py-3 flex items-center justify-between transition-colors outline-none ${openSection === 'inventory' ? 'bg-blue-50/50' : 'bg-slate-50 hover:bg-slate-100'}`}>
                      ...
                      {/* 🚀 Animación de Mantequilla usando Grid */}
                      <div className={`grid transition-all duration-300 ease-in-out ${openSection === 'inventory' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <div className="divide-y divide-slate-100 border-t border-slate-100">
                            <PermissionRow label="Crear Productos" desc="Registrar nueva mercadería en su sucursal." stateKey="canCreateProducts" />
                            <PermissionRow label="Editar Precios y Stock" desc="Modificar productos y actualizar stock." stateKey="canEditProducts" />
                            <PermissionRow label="Ver Stock Externo" desc="Visión de productos en otras tiendas (Solo Lectura)." stateKey="canViewOtherBranches" />
                            <PermissionRow label="Catálogo Global (Modo Dios)" desc="Control absoluto para editar productos de TODAS las tiendas." stateKey="canManageGlobalProducts" type="critical" />
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* CAJA Y VENTAS */}
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                      <button type="button" onClick={() => toggleSection('pos')} className={`w-full px-4 py-3 flex items-center justify-between transition-colors outline-none ${openSection === 'pos' ? 'bg-blue-50/50' : 'bg-slate-50 hover:bg-slate-100'}`}>
                      ...
                      <div className={`grid transition-all duration-300 ease-in-out ${openSection === 'pos' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <div className="divide-y divide-slate-100 border-t border-slate-100">
                            <PermissionRow label="Abrir / Cerrar Turnos" desc="Iniciar y finalizar ventas del día." stateKey="canOpenCloseCash" />
                            <PermissionRow label="Aplicar Descuentos" desc="Modificar el precio final al cobrar." stateKey="canApplyDiscounts" />
                            <PermissionRow label="Ver Total de Caja" desc="Si se desactiva, obliga al empleado a hacer Corte Ciego (no ve lo que debe haber)." stateKey="canViewDailySummary" type="warning" />
                            <PermissionRow label="Anular Ventas / Devoluciones" desc="Permite anular boletas y retornar stock." stateKey="canVoidSales" type="critical" />
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* PRIVACIDAD */}
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                      <button type="button" onClick={() => toggleSection('privacy')} className={`w-full px-4 py-3 flex items-center justify-between transition-colors outline-none ${openSection === 'privacy' ? 'bg-blue-50/50' : 'bg-slate-50 hover:bg-slate-100'}`}>
                      ...
                      <div className={`grid transition-all duration-300 ease-in-out ${openSection === 'privacy' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                        <div className="overflow-hidden">
                          <div className="divide-y divide-slate-100 border-t border-slate-100">
                            <PermissionRow label="Ver Costos de Proveedor" desc="Si se desactiva, solo verá el precio de venta." stateKey="canViewCosts" />
                            <PermissionRow label="Gestionar Clientes" desc="Acceder a teléfonos y correos." stateKey="canManageCustomers" />
                            <PermissionRow label="Ver Estadísticas" desc="Ver Dashboard de ventas y ganancias." stateKey="canViewReports" />
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="px-4 sm:px-6 py-3 bg-white border-t border-slate-200 flex justify-end gap-2 sm:gap-3">
          <Button type="button" variant="outline" onClick={onClose} disabled={isLoading} className="h-9 text-[11px] sm:text-xs font-bold hover:bg-slate-100 text-slate-600">
            Cancelar
          </Button>
          <Button type="submit" form="user-form" disabled={isLoading} className="h-9 text-[11px] sm:text-xs font-bold bg-blue-600 hover:bg-blue-700 text-white px-6 shadow-sm">
