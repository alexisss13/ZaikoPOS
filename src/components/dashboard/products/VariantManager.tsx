...
import { ArrowLeft01Icon, ArrowRight01Icon, PlusSignIcon, Cancel01Icon, Upload02Icon, Delete02Icon, RefreshIcon } from 'hugeicons-react';
...
export function VariantManager({ 
  ...
  const [step, setStep] = useState<'types' | 'generate' | 'attributes'>(isEditing && variants.length > 0 ? 'attributes' : 'types');
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-center px-4">
        <div className="flex items-center space-x-2 md:space-x-4 overflow-x-auto">
          {['types', 'generate', 'attributes'].map((stepKey, index) => (
            <div key={stepKey} className="flex items-center shrink-0">
              <div className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs md:text-sm font-bold ${
                step === stepKey 
                  ...
                  : index < ['types', 'generate', 'attributes'].indexOf(step)
      ...
      {/* Paso 3: Vista tabular para atributos */}
      {step === 'attributes' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-lg font-bold text-slate-900">Paso 3: Configura atributos de variantes</h3>
              <p className="text-slate-600">Personaliza SKU, códigos de barras e imágenes para cada variante</p>
            </div>
              ...
              {variants.map((variant) => (
                <div key={variant.id} className="bg-white border border-slate-200 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                      <Label className="text-xs font-bold text-slate-700 mb-1 block">Código de barras</Label>
                      <Input
                  ...
                  <div>
                    <Label className="text-xs font-bold text-slate-700 mb-1 block">Imágenes (opcional)</Label>
                    <div className="flex items-center gap-1">
                      {variant.images.length > 0 && (
                        <div className="flex gap-1">
                          ...
                          {variant.images.length > 4 && (
                            <div className="w-12 h-12 rounded border border-slate-200 bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600">
                              +{variant.images.length - 4}
                            </div>
                          )}
                        </div>
                      ...
                      {variant.images.length < 4 && (
                        <label className="w-12 h-12 border-2 border-dashed border-slate-300 rounded flex items-center justify-center cursor-pointer hover:border-slate-400">
                          <Upload02Icon className="w-4 h-4 text-slate-400" />
                          <input
                            type="file"
                            accept="image/*"
                            multiple
            ...
            <table className="hidden md:table w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="text-left py-3 px-2 font-bold text-slate-700">Código de barras</th>
                  <th className="text-left py-3 px-2 font-bold text-slate-700 min-w-[120px]">Imágenes (opcional)</th>
                  ...
                  <tr key={variant.id} className="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="py-3 px-2 min-w-[120px]">
                      <div className="flex items-center gap-1">
                        ...
                        {variant.images.length < 4 && (
                          <label className="w-8 h-8 border-2 border-dashed border-slate-300 rounded flex items-center justify-center cursor-pointer hover:border-slate-400">
